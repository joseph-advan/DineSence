...
import asyncio
import httpx
from openai import AsyncOpenAI  # 改為導入 AsyncOpenAI
...
async def summarize_session(stats: dict, store_type: str, tone: str, tips_style: str,
    ...
    # ... (內部邏輯不變，只需要將 client.chat.completions.create 改為 await)
    if client is None:
    ...
    user_prompt = (
        ...
        "### 顧客動態觀察\n"
        "（請依據 timeline 或情緒變化，用 2-3 句話生動地描述顧客可能的用餐過程...）\n\n"
        "### 整體體驗評估\n"
        "（綜合所有數據，歸納顧客本次的整體滿意度趨勢...）\n\n"
        "### actionable 營運建議\n"
        "（根據你的觀察，提出 2-3 點具體、可執行的改善建議。）\n\n"
        "**特別注意**：如果數據中出現『討厭』表情或『剩餘50%以上』的次數總和超過 5 次，請在報告最上方加上一個【⚠️ 警示：需優先關注】的標題..."
    )

    try:
        r = await client.chat.completions.create( # <--- 改為 await
            model=model,
            messages=[
